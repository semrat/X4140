...
MuMuKKPAT::MuMuKKPAT(const edm::ParameterSet& iConfig) :
  ...
  MuMuKKMinMass(iConfig.getUntrackedParameter<double>("MinMuMuKKMass", 0)),
  MuMuKKMaxMass(iConfig.getUntrackedParameter<double>("MaxMuMuKKMass", 10)),
...

{
  ...
  MuMuKKMaxMass = JPsiPhiMaxXMass; 
  MuMuKKMinMass = JPsiPhiMinBs0Mass; 
  MuMuKKMaxMass = JPsiPhiMaxBs0Mass;
}
  ...
  if ( (doMC && !MCExclusiveDecay) || (doMC && (MCExclusiveDecay && decayChainOK)) || doData ) {
	...
	for ( std::vector<pat::Muon>::const_iterator Muon1 = thePATMuonHandle->begin(); Muon1 != thePATMuonHandle->end(); ++Muon1 ) {
	  ...
	  for ( std::vector<pat::Muon>::const_iterator Muon2 = Muon1+1; Muon2 != thePATMuonHandle->end(); ++Muon2) {
	    ...
	    ////////////////// get the MuMu information & KaKa information //////////////////	
	    TransientTrack muon1TT( muTrack1, &(*bFieldHandle) );
	    TransientTrack muon2TT( muTrack2, &(*bFieldHandle) );			
	    KinematicParticleFactoryFromTransientTrack pFactory;
    	    TransientTrack kaon1TT( kaTrack1, &(*bFieldHandle) ); /// SEMRA for kaon-kaon 
            TransientTrack kaon2TT( kaTrack2, &(*bFieldHandle) ); 

	    /// initial chi2 and ndf before kinematic fits
	    float chi = 0., ndf = 0.;
	    vector<RefCountedKinematicParticle> muons; /// SEMRA the final state muons produced by the KinematicParticleFactory / The sequential fit 
	    ...
	    vector<RefCountedKinematicParticle> kaons; /// SEMRA the final state kaons produced by the KinematicParticleFactory
	    ...
	    KinematicParticleVertexFitter MuMuFitter; /// SEMRA creating the vertex fitter for JPsi
	    RefCountedKinematicTree MuMuVertexFitTree; 
	    MuMuVertexFitTree = MuMuFitter.fit(muons); 
	    KinematicParticleVertexFitter KaKaFitter; /// SEMRA creating the vertex fitter Phi
	    RefCountedKinematicTree KaKaVertexFitTree; 
	    MuMuVertexFitTree = KaKaFitter.fit(kaons); 
	    if (!MuMuVertexFitTree->isValid()) /// SEMRA for muon-muon
	      continue ; 
	   if (!KaKaVertexFitTree->isValid()) /// SEMRA for kaon-kaon
              continue ;
	    MuMuVertexFitTree->movePointerToTheTop(); /// SEMRA for muon-muon				
	    ...
	    KaKaVertexFitTree->movePointerToTheTop(); /// SEMRA for kaon-kaon
	    ...
	    ////////////////// fill the MuMu vectors & KaKa vectors //////////////////
	   ...
	   if (KaKaCand_fromFit->currentState().mass() < KKMinMass  ||  KaKaCand_fromFit->currentState().mass() > KKMaxMass) /// SEMRA for Phi
              continue ;
            ...
            ka1Idx->push_back(std::distance(thePATKaonHandle->begin(), Kaon1));
            ka2Idx->push_back(std::distance(thePATKaonHandle->begin(), Kaon2));
	    ...
	    ka1_KaKa_Px->push_back( Ka1Cand_KP.momentum().x()); /// SEMRA for Phi
	    ...
	    /// SEMRA for Phi
            ...
            cout << dikaonType <<endl;

            if (Debug_) cout << "evt:" << evtNum <<" KaKa with diKaonType = " << dikaonType <<endl;
            cout << "POINT 1" << endl; 
            KaKaType->push_back(dikaonType);
            cout << "POINT 2" << endl; 
              ...
              { 
                ...
                if ( MatchingTriggerResult[MatchTrig]!=0 )
                  { 
                    ...
                    cout << "FILTER WAS OK" << endl; 
                    pat::TriggerObjectStandAloneCollection ka1HLTMatches = Kaon1->triggerObjectMatchesByFilter( FiltersForMatching_[MatchTrig] );
                    cout << "MUON 1 OK :)" << endl; 
                    pat::TriggerObjectStandAloneCollection ka2HLTMatches = Kaon2->triggerObjectMatchesByFilter( FiltersForMatching_[MatchTrig] );
                    cout << "POINT 3" << endl;
                    bool pass1 = ka1HLTMatches.size() > 0;
                    bool pass2 = ka2HLTMatches.size() > 0;
                    cout << "POINT 4" << endl;           
                    if ((pass1) && (pass2))
                      { 
                        cout << " SECOND IF" << endl; 
                        KaKaKaonTrigMatch->push_back(true);
                        if (Debug_) cout <<"Matched MuMu" <<endl ;
                      } else
                      cout << "THIRD ELSE" << endl; 
                      KaKaKaonTrigMatch->push_back(false);
                  }
                else
	...
	/* 
	    ...
	    if (MuMuMass->at(nMuMu-1) < MuMuMinMass  ||  MuMuMass->at(nMuMu-1) > MuMuMaxMass)
 	    ...
 	    /*////////////////// cuts on MuMu mass window for X(4140) //////////////////
 	    if (MuMuMass->at(nMuMu-1) < MuMuMinMass  ||  MuMuMass->at(nMuMu-1) > MuMuMaxMass)
 	    continue ; nX_pre1++ ;*/  
	   ...
	   for ( vector<pat::GenericParticle>::const_iterator Track2 = theKaonRefittedPATTrackHandle->begin(); Track2 != theKaonRefittedPATTrackHandle->end(); ++Track2 ){
	    ...
	    /// cuts on charged tracks	
		  ...
		  float MuMuKDR = sqrt( pow(MuMu.eta() - Track1->p4().eta(),2) + pow(MuMu.phi() - Track1->p4().phi(), 2) );
		  //float MuMuK2DR = sqrt( pow(MuMu.eta() - Track2->p4().eta(),2) + pow(MuMu.phi() - Track2->p4().phi(), 2) );
		  float bs0KDR = sqrt( pow(bs0.eta() - Track1->p4().eta(),2) + pow(bs0.phi() - Track1->p4().phi(), 2) );
                  //float bs0K2DR = sqrt( pow(MuMu.eta() - Track2->p4().eta(),2) + pow(MuMu.phi() - Track2->p4().phi(), 2) );
   		  float xKDR = sqrt( pow(x.eta() - Track1->p4().eta(),2) + pow(x.phi() - Track1->p4().phi(), 2) );
                  //float xK2DR = sqrt( pow(MuMu.eta() - Track2->p4().eta(),2) + pow(MuMu.phi() - Track2->p4().phi(), 2) );		  

		  if (UseBs0DR) {
		    if (bs0KDR > Bs0TrackMaxDR) /// ||  bs0K2DR > Bs0TrackMaxDR)
		      continue ; // B0TrackMaxDR = 2 
		  } else {
		    if (MuMuKDR > MuMuTrackMaxDR) ///  ||  MuMuK2DR > MuMuTrackMaxDR)
		      continue ; // MuMuTrackMaxDR = 3.5
		  }
		  nBs0_pre8++ ; 
 
		  if (UseXDR) {
		    if ( xKDR > XTrackMaxDR) /// || xK2DR > XTrackMaxDR)
		      continue ;
                  } else {
		    if (MuMuKDR > MuMuTrackMaxDR) ///  ||  MuMuK2DR > MuMuTrackMaxDR)
		      continue ;
		  ...
		  if ((Track1->p4() + Track2->p4() + MuMu).M() > MuMuKKMaxMass  ||  (Track1->p4() + Track2->p4() + MuMu).M() < MuMuKKMinMass)
		    continue ; nBs0_pre9++ ; /// SEMRA ask
		  //if ((Track1->p4() + Track2->p4() + MuMu).M() > MuMuKKMaxMass  ||  (Track1->p4() + Track2->p4() + MuMu).M() < MuMuKKMaxMass)
		    //continue ; nX_pre9++ ; /// SEMRA ask 
		 ...
		 if (doMuMuMassConst) { // MassConst = 'MC' in the following
		  ...
		  Bs0_k1Idx->push_back(std::distance(thePATTrackHandle->begin(), Track1)); 
		  ...
		  X_MuMuIdx->push_back(nMuMu-1);
                  X_k1Idx->push_back(std::distance(thePATTrackHandle->begin(), Track1)); 
...
void MuMuKKPAT::beginJob()
{
  ...
  Bs0_One_Tree_->Branch("nKaKa",&nMuMu,"nKaKa/i");
  Bs0_One_Tree_->Branch("KaKaMass",&KaKaMass);
  Bs0_One_Tree_->Branch("KaKaPx",&KaKaPx);
  Bs0_One_Tree_->Branch("KaKaPy",&KaKaPy);
  Bs0_One_Tree_->Branch("KaKaPz",&KaKaPz);
  Bs0_One_Tree_->Branch("KaKaVtx_CL",&KaKaVtx_CL);
  Bs0_One_Tree_->Branch("KaKaVtx_Chi2",&KaKaVtx_Chi2);
  Bs0_One_Tree_->Branch("KaKaDecayVtx_X",&KaKaDecayVtx_X);
  Bs0_One_Tree_->Branch("KaKaDecayVtx_Y",&KaKaDecayVtx_Y);
  Bs0_One_Tree_->Branch("KaKaDecayVtx_Z",&KaKaDecayVtx_Z);
  Bs0_One_Tree_->Branch("KaKaDecayVtx_XE",&KaKaDecayVtx_XE);
  Bs0_One_Tree_->Branch("KaKaDecayVtx_YE",&KaKaDecayVtx_YE);
  Bs0_One_Tree_->Branch("KaKaDecayVtx_ZE",&KaKaDecayVtx_ZE);
  /// muons from JPsi (MuMu) fit & kaons from Phi (KaKa) fit
  ...
  Bs0_One_Tree_->Branch("ka1Idx",&ka1Idx);
  Bs0_One_Tree_->Branch("ka2Idx",&ka2Idx);
  Bs0_One_Tree_->Branch("ka1Px_MuMu",&ka1_KaKa_Px);
  Bs0_One_Tree_->Branch("ka1Py_MuMu",&ka1_KaKa_Py);
  Bs0_One_Tree_->Branch("ka1Pz_MuMu",&ka1_KaKa_Pz);
  Bs0_One_Tree_->Branch("ka1Chi_MuMu2",&ka1_KaKa_Chi2);
  Bs0_One_Tree_->Branch("ka1NDF_MuMu",&ka1_KaKa_NDF);
  Bs0_One_Tree_->Branch("ka2Px_MuMu",&ka2_KaKa_Px);
  Bs0_One_Tree_->Branch("ka2Py_MuMu",&ka2_KaKa_Py);
  Bs0_One_Tree_->Branch("ka2Pz_MuMu",&ka2_KaKa_Pz);
  Bs0_One_Tree_->Branch("ka2Chi2_MuMu",&ka2_KaKa_Chi2);
  Bs0_One_Tree_->Branch("ka2NDF_MuMu",&ka2_KaKa_NDF);
  Bs0_One_Tree_->Branch("KaKaType",&KaKaType);
  Bs0_One_Tree_->Branch("KaKaKaonTrigMatch",&KaKaKaonTrigMatch);
  /// Primary Vertex with "MuMu correction" 
