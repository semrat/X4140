...
UInt_t FourOnia::isTriggerMatched(const pat::Muon* muon) {
...
// if matched a given trigger, set the bit, in the same order as listed
  for (unsigned int iTr = 0; iTr<HLTFilters_.size(); iTr++ ) {
     ...
     const pat::TriggerObjectStandAloneCollection muHLTMatch = muon->triggerObjectMatchesByFilter(HLTFilters_[iTr]);
     if (!muHLTMatch.empty()) matched += (1<<iTr);
  }
...
FourOnia2MuMuPAT::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(View<pat::Muon>::const_iterator it1 = muons->begin(), itend = muons->end(); it1 != itend; ++it1){
    // both must pass low quality
    if(!lowerPuritySelection_(*it1)) continue;
      ...
      if(!((it1->charge() + it2->charge())==0))
        ...
        for(View<pat::Muon>::const_iterator it3 = muons->begin(), itend = muons->end(); it3 != itend; ++it3){
          ...
          for(View<pat::Muon>::const_iterator it4 = muons->begin(), itend = muons->end(); it4!= itend; ++it4){
            ...
            if(!((it1->charge() + it2->charge() + it13->charge() + it4->charge())==0))
            continue;

            std::vector<pat::Muon> theMuons;
            std::vector< std::pair <pat::Muon,pat::Muon> > zeroChargeMuMU;
            theMuons.push_back(*it1);
            theMuons.push_back(*it2);
            theMuons.push_back(*it3);
            theMuons.push_back(*it4);

            for (size_t i = 0;() i < theMuons.size(); i++)
              if(isTriggerMatched(dynamic_cast<const pat::Muon>(theMuons[i])));
            for (size_t i = 0; i < theMuons.size(); i++)
              for (size_t j = i+1; j < theMuons.size(); j++)
                if((theMuons[i].charge() + theMuons[j].charge())==0)
                  zeroChargeMuMU.push_back(std::pair(theMuons[i],theMuons[j]));
          ...
          myCand.addDaughter(*it1, "muon1");
          myCand.addDaughter(*it2,"muon2");

          // ---- define and set candidate's 4momentum  ----
          LorentzVector mumu = it1->p4() + it2->p4();
          myCand.setP4(mumu);
          myCand.setCharge(it1->charge()+it2->charge());
	...
	t_tks.push_back(theTTBuilder->build(*it1->track()));  // pass the reco::Track, not  the reco::TrackRef (which can be transient)
	...
	CachingVertex<5> VtxForInvMass = vtxFitter.vertex( t_tks );
        ...
        if ( field->nominalValue() > 0 ) {
          ...
          myVertex = TransientVertex();                      // with no arguments it1 is invalid
        }
	...
	if (myVertex.isValid()) {
	  ...
	  if( addMuonlessPrimaryVertex_  && thePrimaryV.tracksSize()>2) {
	    // Primary vertex matched to the dimuon, now refit it1 removing the two muons
	    ...
	    iEvent.getByToken(revtxtrks_,   pvtracks);
 	    if( !pvtracks.isValid()) { std::cout << "pvtracks NOT valid " << std::endl; }
	      ...
	      const reco::Muon *rmu1 = dynamic_cast<const reco::Muon *>(it1->originalObject());
      ...
      // ---- MC Truth, if enabled ----
      if (addMCTruth_) {
	reco::GenParticleRef genMu1 = it1->genParticleRef();
