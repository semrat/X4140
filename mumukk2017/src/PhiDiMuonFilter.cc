...
class PhiDiMuonFilter : public edm::EDProducer {
 public:
  explicit PhiDiMuonFilter(const edm::ParameterSet&);
  ~PhiDiMuonFilter() override {};
...
PhiDiMuonFilter::PhiDiMuonFilter(const edm::ParameterSet& iConfig):
...
UInt_t PhiDiMuonFilter::isTriggerMatched(const pat::CompositeCandidate *diMuon_cand) {
...
void PhiDiMuonFilter::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
DEFINE_FWK_MODULE(PhiDiMuonFilter);
