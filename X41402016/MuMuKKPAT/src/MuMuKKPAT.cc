// -*- C++ -*-
//
// Package:    mumuKaonKaon
// Class:      mumuKaonKaon
//
/**\class mumuKaonKaon mumuKaonKaon.cc myAnalyzers/mumuKaonKaon/src/mumuKaonKaon.cc
...
/// user include files
#include "../interface/mumuKaonKaon.h"
...
///
mumuKaonKaon::mumuKaonKaon(const edm::ParameterSet& iConfig) :
...
HLTTriggersListPyc(iConfig.getUntrackedParameter<std::vector<std::string> >("TriggersForMatching")),
...
class mumuKaonKaon : public edm::EDAnalyzer {
public:
  explicit mumuKaonKaon(const edm::ParameterSet&);
  ~mumuKaonKaon() override {};
...
void mumuKaonKaon::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  } ///END triggerResults_handle.isValid()
    ...
    if (thePATMuonHandle->size() >= 2  && hasRequestedTrigger) {
      ...
      for ( std::vector<pat::Muon>::const_iterator Muon1 = thePATMuonHandle->begin(); Muon1 != thePATMuonHandle->end(); ++Muon1 ) {
        ...
        for ( std::vector<pat::Muon>::const_iterator Muon2 = Muon1+1; Muon2 != thePATMuonHandle->end(); ++Muon2) {
          ...
          int ntriggers = HLTTriggersListPyc.size();
          if (Debug_) cout << "ntriggers: " << ntriggers << endl;
          for (int MatchTrig = 0; MatchTrig < ntriggers; MatchTrig++)
          {
            if (Debug_) cout << "HLTMatchFlag[" << MatchTrig << "]: " << HLTMatchFlag[MatchTrig] << endl;
            if ( HLTMatchFlag[MatchTrig]!=0 )
            {
              //cout << "POINT 3" << endl;
          ...
          void mumuKaonKaon::beginRun(edm::Run const & iRun, edm::EventSetup const& iSetup)
          {
          }
          void mumuKaonKaon::beginJob()
          {
          ...
          void mumuKaonKaon::endJob() {
            X_One_Tree_->GetDirectory()->cd();
            X_One_Tree_->Write();
          }/// endjob


          bool mumuKaonKaon::isAbHadron(int pdgID) {
          ...
          bool mumuKaonKaon::isAMixedbHadron(int pdgID, int momPdgID) {
          ...
          std::pair<int, float> mumuKaonKaon::findCandMCInfo(reco::GenParticleRef genCand) {
          ...
          double mumuKaonKaon::getSigmaOfLogdEdx(double logde)
          {
            return 0.3;
          }

          float mumuKaonKaon::getEnergyLoss(const reco::TrackRef & track)
          {
          ...
          double mumuKaonKaon::nsigmaofdedx(const reco::TrackRef & track, double & theo, double & sigma)
          {
          ...
          double mumuKaonKaon::getLogdEdx(double bg)
          {
            const double a =  3.25 ;
          ...
          double mumuKaonKaon::GetMass(const reco::TrackRef & track){
            double P = track->p();
            double C = 2.625;
            double K = 2.495;
          ...
          template<typename T>
          bool mumuKaonKaon::isBetterMuon(const T &mu1, const T &mu2) const {
          ...
          bool mumuKaonKaon::isSameMuon(const reco::Muon &mu1, const reco::Muon &mu2) const {
            return (& mu1 == & mu2) ||
...
void mumuKaonKaon::fillDescriptions(edm::ConfigurationDescriptions & descriptions) {
...
DEFINE_FWK_MODULE(mumuKaonKaon);
