...
FourOniaPAT::FourOniaPAT(const edm::ParameterSet& iConfig):
...
UInt_t FourOniaPAT::isTriggerMatched(pat::Muon *muonCandidate) {
  ...
  for (unsigned int iTr = 0; iTr<HLTFilters_.size(); iTr++ ) {
    ...
    const pat::TriggerObjectStandAloneCollection muMatches = muonCandidate->triggerObjectMatchesByFilter(HLTFilters_[iTr]);
    if (!muMatches.empty()) matched += (1<<iTr);
...
void
FourOniaPAT::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
  ...
  for(View<pat::Muon>::const_iterator it = muons->begin(), itend = muons->end(); it != itend; ++it){
    ...
    bool
    FourOniaPAT::isAbHadron(int pdgID) {
    ...
    bool
    FourOniaPAT::isAMixedbHadron(int pdgID, int momPdgID) {
    ...
    std::pair<int, float>
    FourOniaPAT::findJpsiMCInfo(reco::GenParticleRef genJpsi) {
