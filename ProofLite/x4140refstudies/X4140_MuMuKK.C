...
void X4140_MuMuKK::SlaveBegin(TTree * /*tree*/)
{
  ...
  std::string xcandHistoDM = "Xcand_histo_DM_";
  ...
  refKKvsKK = new TH2F ("refKKvsKK","refKKvsKK",1000, 0.0, 2.0,1000, 0.0, 2.0);
  refK1vsK1 = new TH2F ("refK1vsK1","refK1vsK1",1000, 0.0, 2.0,1000, 0.0, 2.0);
  refK2vsK2 = new TH2F ("refK2vsK2","refK2vsK2",1000, 0.0, 2.0,1000, 0.0, 2.0);

  refMuMuvsMuMu = new TH2F ("refMuMuvsMuMu","refMuMuvsMuMu",1000, 2.5, 3.5,1000, 2.5, 3.5);
  refMu1vsMu1   = new TH2F ("refMu1vsMu1","refMu1vsMu1",1000, 0.0, 3.0,1000, 0.0, 3.0);
  refMu2vsMu2   = new TH2F ("refMu2vsMu2","refMu2vsMu2",1000, 0.0, 3.0,1000, 0.0, 3.0);
...
bool X4140_MuMuKK::Process(Long64_t entry)
{
  ...
  bool trueFalse[2] = {true,true};

  TH1F* x5586_hists[2] = {X5568_Cand_Mass,X5568_Cand_Mass_Ref};
  std::map< std::tuple < std::string, std::string, std::string, std::string >,TH1F*> xCand_hists[1] = {XCandMassHistos};//,XCandMassHistos_Ref};
  std::map< std::tuple < std::string, std::string, std::string, std::string >,TH1F*> jPsis_hists[1] = {JPsiMassHistos};//,JPsiMassHistos_Ref};
  std::map< std::tuple < std::string, std::string, std::string, std::string >,TH1F*> phi_hists[1]   = {PhiMassHistos};//,PhiMassHistos_Ref};
  ...
  for(int iRef=0; iRef<1; ++iRef)
    {
      ...
      for(int iX=0; iX<nX; ++iX)
      {
        ...
        TLorentzVector kaon1, kaon2,originalK1,originalK2;
        ...
        double mkk = (originalK1+originalK2).M(), mmumu = (originalmu1+originalmu2).M();

        refKKvsKK->Fill(Phi.M(),mkk);
        refK1vsK1->Fill(kaon1.M(),originalK1.M());
        refK2vsK2->Fill(kaon2.M(),originalK2.M());

        refMuMuvsMuMu->Fill(JPsi.M(),mmumu);
        refMu1vsMu1->Fill(mu1.M(),originalmu1.M());
        refMu2vsMu2->Fill(mu2.M(),originalmu2.M());
        ...
        jPsiMassCut    =  (JPsi.M() < 3.4) && (JPsi.M() > 2.8);
        ...
        phiCut = ((Phi.M()) > 1.013 && (Phi.M()) < 1.026);
                  ...
                  if(regsFlags[k])
                  {
                    ...
                    for (size_t i = 0; i < cutsFlags.size(); ++i)
                    {
                      ...
                      if(cut)
                      {
                        ...
                        XCandMassHistosDeltaM[histokey]->Fill(XCand.M()-Phi.M()+Phi_mass);
                      }
                    }
      ...
      for (size_t j = 0; j < 3; ++j)
      {
        for (size_t k = 0; k < 4; ++k)
        {
          for (size_t i = 0; i < 6; ++i)
          {
            ...
            XCandMassHistosDeltaM[histokey]->Write();
